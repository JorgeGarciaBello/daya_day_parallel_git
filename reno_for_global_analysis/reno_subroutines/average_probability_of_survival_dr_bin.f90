!##################################################################
!
!   averageProbabilityOfSurvival_dr_bin: is the average 
...
!
!##################################################################
real(8) function averageProbabilityOfSurvival_dr_bin(d,r,bin)
    ...
    h = (b-a)/real(n)   
    averageProbabilityOfSurvival_dr_bin=0.d0
    do i=1,n
        x = a + h*real(i-1)
        x = x*(1.0d0 + engyPull)
        averageProbabilityOfSurvival_dr_bin = averageProbabilityOfSurvival_dr_bin &
            + h*( probability(t13,dm31,leng,x) &
                ...
                / real(2)
    enddo
    averageProbabilityOfSurvival_dr_bin = averageProbabilityOfSurvival_dr_bin/(b-a)
    return
end function averageProbabilityOfSurvival_dr_bin