!#########################################################
!
!   sk_data_plot_sk_db_reno: It is a subroutine that
!       create the point to plot three different 
!       profiles, such as, (chi2,dm32), (chi2,s2t13) 
!       and (chi2,s2t23).!
!
!#########################################################
subroutine sk_data_plot_sk_db_reno(case_plotter)
    ...
    open(newunit=u, file='sk-db-reno-minos-ji-nh.dat')
        do i=1,n
            write(u,*) data(i,:)
        enddo
    close(u)
    ...
    select case(case_plotter)    
        case(1) ! Opción para generar datos de la gráfica (dm_32,chi)
            open(newunit=u, file='sk-data-files/sdrm_dm_32_vs_chi_'//trim(data_file)//'.dat')
            do i=1,n
        ...
        case(2) ! Opción para generar datos de la gráfica (sin2t13,chi)
            open(newunit=u, file='sk-data-files/sdrm_sin2t13_vs_chi_'//trim(data_file)//'.dat')
            do i=1,n
        ...
        case(3) ! Opción para generar datos de la gráfica (sin2t23,chi)
            open(newunit=u, file='sk-data-files/sdrm_sin2t23_vs_chi_'//trim(data_file)//'.dat')
            do i=1,n
